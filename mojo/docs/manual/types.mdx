...
types. The `Scalar` type is a `SIMD` vector with a single element. The numeric
types listed in [Table 1](#table-1), like `Int`, `Int8`, and `Float32`, are type
...
[`Copyable`](/docs/std/traits/copyable/Copyable/) trait. Most of the common
...
must conform to the [`Copyable`](/docs/std/traits/copyable/Copyable/) trait.
