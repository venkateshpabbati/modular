...
from its `Storage` parameter (`PointerStorage[element_width=N]`). For more
...
(`Storage=PointerStorage[element_width=1]`), so you can cast the element value
directly to an `Int`.

```mojo
def takes_scalar_tensor(
    tensor: TileTensor[.int32, Storage=PointerStorage[element_width=1], ...]
) -> Int:
...
[`lane_id()`](https://mojolang.org/docs/std/gpu/primitives/id/lane_id/) as the
