...
block movement across memory tiers and replicas.
...
| `maxserve_cache_used_host_kv_pct_percent`                 | Histogram | Percentage of host KV cache blocks in use (0-100%), sampled once per scheduler batch when host paging is enabled.                                                                                                              |
| `maxserve_cache_used_disk_kv_pct_percent`                 | Histogram | Percentage of disk KV cache blocks in use (0-100%), sampled once per scheduler batch when disk paging is enabled.                                                                                                              |
...
| `maxserve_cache_h2d_blocks_copied_blocks_total`           | Counter   | Cumulative host-to-device KV block copies.                                                                                                                                                                                     |
| `maxserve_cache_d2h_blocks_copied_blocks_total`           | Counter   | Cumulative device-to-host KV block copies.                                                                                                                                                                                     |
...
| `maxserve_cache_disk_blocks_read_blocks_total`            | Counter   | Cumulative KV blocks read from the disk cache tier.                                                                                                                                                                            |
| `maxserve_cache_disk_blocks_written_blocks_total`         | Counter   | Cumulative KV blocks written to the disk cache tier.                                                                                                                                                                           |
...
The following metrics track NIXL transfer performance and dKV client
connectivity.

| Metric                                             | Type      | Description                                                                                              |
|----------------------------------------------------|-----------|----------------------------------------------------------------------------------------------------------|
| `maxserve_dkv_nixl_read_latency_milliseconds`      | Histogram | NIXL read transfer latency, in milliseconds.                                                             |
| `maxserve_dkv_nixl_write_latency_milliseconds`     | Histogram | NIXL write transfer latency, in milliseconds.                                                            |
| `maxserve_dkv_nixl_read_gib_per_s_GiB_per_second`  | Histogram | NIXL read throughput, in GiB/s.                                                                          |
| `maxserve_dkv_nixl_write_gib_per_s_GiB_per_second` | Histogram | NIXL write throughput, in GiB/s.                                                                         |
| `maxserve_dkv_rpc_acquire_latency_milliseconds`    | Histogram | dKV `acquire_blocks()` RPC latency, in milliseconds.                                                     |
| `maxserve_dkv_rpc_read_latency_milliseconds`       | Histogram | dKV `read_blocks()` RPC latency, in milliseconds.                                                        |
| `maxserve_dkv_connected_clients`                   | Gauge     | Number of dKV connector clients currently connected to the external KV tier.                             |
| `maxserve_dkv_total_clients`                       | Gauge     | Total number of dKV connector clients, one per data-parallel replica.                                    |
| `maxserve_dkv_read_blocks_total`                   | Counter   | Cumulative KV blocks landed in device memory from the dKV tier. Only confirmed-complete transfers count. |
| `maxserve_dkv_reconnect_attempts`                  | Gauge     | Cumulative dKV reconnect attempts across all clients.                                                    |
