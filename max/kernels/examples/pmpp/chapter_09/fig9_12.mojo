...
def histogram_kernel(
    ...
    """GPU histogram kernel with coarsening (contiguous partitioning).
    ...
    # Coarsened processing with contiguous partitioning
    var segment = COARSE_FACTOR * UInt32(block_idx.x) * UInt32(256)
    for c in range(COARSE_FACTOR):
        var i = segment + UInt32(thread_idx.x) * COARSE_FACTOR + UInt32(c)
        if i < width * height:
            var bin = image[Int(i)]
...
def main() raises:
    """Example usage of GPU histogram with coarsening (contiguous)."""
