...
def stencil_kernel(
    ...
    the z-direction. Only 1 z-plane is kept in shared memory (for x-y neighbors),
    while z-neighbors (prev/next) are kept in registers per thread.
    This reduces shared memory usage by 3× compared to full shared memory tiling.
    ...
    # Load prev plane (i_start - 1) into REGISTER
    if (
        i_prev >= 0
        and i_prev < N
        and in_row >= 0
        ...
        prev = d_in[global_idx]

    # Load curr plane (i_start) into SHARED MEMORY
    if (
        i_start >= 0
        and i_start < N
        and in_row >= 0
    ...
    for in_z in range(i_start, i_start + OUT_TILE_DIM):
        ...
        if 1 <= in_z < N - 1 and 1 <= in_row < N - 1 and 1 <= in_col < N - 1:
            if (
                ...
                # Calculate stencil: x-y neighbors from shared, z-neighbors from registers
                ...
                result += d_c[5] * prev  # i-1 (from register!)
                result += d_c[6] * next  # i+1 (from register!)
        ...
        # Rotate buffers: prev <- curr (from shared), curr <- next (from register)
        prev = Float32(curr_in_s[shared_idx])
        curr_in_s[shared_idx] = next
...
def main() raises:
    ...
    print("3D Stencil with REGISTER TILING")
    print("=" * 50)
    ...
    print("Launching GPU kernel with register tiling...")
    print(
        "Grid:",
        "(",
        grid_dim_x,
