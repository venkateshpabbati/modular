...
def scan_kernel(
    ...
    """Scan kernel with register tiling and thread coarsening.
    ...
    buffer_r[0] = buffer_s[thread_segment]

    comptime for c in range(1, COARSE_FACTOR):
        buffer_r[c] = buffer_s[thread_segment + c] + buffer_r[c - 1]
    ...
    comptime for c in range(COARSE_FACTOR):
        buffer_s[thread_segment + c] = buffer_r[c] + prev_partial_sum
...
def main() raises:
    """Example usage of scan kernel with register tiling."""
    ...
    print(
        "Launching scan with register tiling (Fig 11.13) with 1 block,",
        BLOCK_DIM,
    ...
    with DeviceContext() as ctx:
        ...
        # Launch kernel (single block with register tiling)
