...
"""Correctness test for SM100 MHA with native FP8 Q, K, V — DECODE (head_dim=512).

Sibling of `test_mha_sm100_qkv_fp8_d{128,256}_decode.mojo` at head_dim=512
(Gemma4-global decode shape). At d=512, `mha_1q.mojo` takes the
`use_p_smem` branch: P is written to SMEM (rather than TMEM) and P@V
becomes an SS MMA. The fp32→fp8 P SMEM write follows the same
byte-packed pattern `mha_depth512/softmax_warp.mojo` uses for its
prefill counterpart.
...
def execute_pure_fp8_decode_test[
    ...
    *,
    ...
    print(
        "test_mha_sm100_qkv_fp8_d512_decode: ",
        "mode=",
        mode_name,
        " mask=",
        mask_name,
...
def main() raises:
    with DeviceContext() as ctx:
        ...
        comptime for i in range(2):
            ...
            # Production Gemma4-global decode shape: head_dim=512, group=8.
        ...
        print("test_mha_sm100_qkv_fp8_d512_decode: ALL PASSED")
