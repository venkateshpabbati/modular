...
"""Correctness test for SM100 MHA with native FP8 Q, K, V — DECODE (head_dim=128).

Pure-FP8 decode path: at seq_len=1 with any head_dim that is a multiple of
64, `flash_attention` lands in `mha_1q.mojo` on SM100. Q, K, V are ALL fp8
e4m3fn; the kernel runs native fp8 UMMA (`KIND_F8F6F4`, MMA_K=32) for both
Q@K^T and P@V. P is cast to fp8 inside the softmax warp.

Each config runs in two modes:

* `bf16_to_fp8=False` (kernel-correctness): generate Q, K, V as fp8 with
  `randn`, then cast fp8 → bf16 (lossless) for the reference inputs.
  Both kernels see identical numerical values, so the cosine isolates
  the fp8 MMA's compute correctness. Bar: cosine ≥ 0.9997.
* `bf16_to_fp8=True` (production-realism): generate Q, K, V as bf16
  with `randn`, then cast bf16 → fp8 (lossy) for the kernel inputs.
  The bf16 reference sees the originals; the fp8 kernel sees the
  quantized values. Bundles quantization error + kernel error into a
  single cosine. Bar: cosine ≥ 0.998.
...
def execute_pure_fp8_decode_test[
    ...
    *,
    ...
    print(
        "test_mha_sm100_qkv_fp8_d128_decode: ",
        "mode=",
        mode_name,
        " mask=",
        mask_name,
...
def main() raises:
    with DeviceContext() as ctx:
        ...
        print("test_mha_sm100_qkv_fp8_d128_decode: ALL PASSED")
