...
"""Correctness test for SM100 MHA with native FP8 Q, K, V (head_dim=512).
...
Sibling of `test_mha_sm100_qkv_fp8_d256_prefill.mojo` at head_dim=512 —
exercises the d=512 pair-CTA kernel with `split_o=True` (O split into
O_lo / O_hi, V split into V_lo / V_hi) at the Gemma4-global shape:

    n_kv_heads          = 4
    head_dim            = 512

Comparison pattern mirrors `test_mla_decode_qkv_fp8.mojo`:
...
4. Test target: `flash_attention` with the fp8 inputs — dispatches to
   `mha_sm100_depth512_dispatch` which uses the fp8 MMA path.
5. Element-wise compare with `atol=3e-1, rtol=5e-2`.
...
# ===-----------------------------------------------------------------------===#
# Core test (parametric on head_dim so the same body covers d=512)
# ===-----------------------------------------------------------------------===#
...
def execute_pure_fp8_test[
    ...
    *,
    ...
    print(
        "test_mha_sm100_qkv_fp8_d512_prefill: ",
        "mask=",
        mask_name,
        " group=",
        group,
    ...
    # the ~1e-4 drop from 1.0 comes from the P → fp8 cast in softmax
    # (P values in [0, 1] lose precision near 0 when cast to e4m3).
    # 0.9997 leaves ~7.5e-5 headroom below the worst observed cosine.
...
def main() raises:
    with DeviceContext() as ctx:
        ...
        # Sliding-window variants at the Gemma4-global shape.
        ...
        print("test_mha_sm100_qkv_fp8_d512_prefill: ALL PASSED")
