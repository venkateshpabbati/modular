...
"""Correctness test for SM100 MHA with native FP8 Q, K, V (head_dim=64).

Sibling of `test_mha_sm100_qkv_fp8_d128_prefill.mojo`. At d=64 the
dispatch still routes to `SM100MHA2Q` (the FA4 2Q prefill kernel) in
single-CTA mode (pair-CTA is gated to `depth > 64`).

1. Generate Q, K, V as fp8 directly on the host with `randn`.
2. Cast fp8 -> bf16 on the host (lossless) for the reference inputs.
3. Reference: `flash_attention` with the bf16 inputs.
4. Test target: `flash_attention` with the fp8 inputs -- dispatches to
   `mha_sm100_2q_dispatch` (`SM100MHA2Q`) with `KIND_F8F6F4` MMAs.
...
def execute_pure_fp8_test[
    ...
    *,
    ...
    print(
        "test_mha_sm100_qkv_fp8_d64_prefill: ",
        "mask=",
        mask_name,
        " group=",
        group,
    ...
    # Cosine >= 0.9997 is the pass/fail gate -- same bar as d128/d256/d512.
...
# ===-----------------------------------------------------------------------===#
# Entry point -- coverage matrix for d=64
# ===-----------------------------------------------------------------------===#
...
def main() raises:
    with DeviceContext() as ctx:
        ...
        # Small-head GQA shapes (d=64 is the typical "small head" prefill
        # config -- e.g. Llama2-7B uses d=128, but smaller / older models
        # and some attention-head splits land here).
        ...
        print("test_mha_sm100_qkv_fp8_d64_prefill: ALL PASSED")
